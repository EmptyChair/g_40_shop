...
public class TokenService {
    ...
    public TokenService(
            @Value("${key.access}") String accessSecretPhrease,
            @Value("${key.refresh}") String refreshSecretPhrease,
            RoleRepository roleRepository) {
        this.accessKey = Keys.hmacShaKeyFor(Decoders.BASE64.decode(accessSecretPhrease));
        this.refreshKey = Keys.hmacShaKeyFor(Decoders.BASE64.decode(refreshSecretPhrease));
        this.roleRepository = roleRepository;
    }

    public String generateAccessToken(User user) {
        LocalDateTime now = LocalDateTime.now();
        Instant expiration = now.plusDays(7).atZone(ZoneId.systemDefault()).toInstant();
    ...
    public String generateRefreshToken(User user) {
        LocalDateTime now = LocalDateTime.now();
        Instant expiration = now.plusDays(30).atZone(ZoneId.systemDefault()).toInstant();
        Date expirationDate = Date.from(expiration);
